...
b[0, 0] = gt
for i in xrange(1, Lr, 1):
    b[i, 0] = float(b[i - 1, 0] + gt)

b[Lr, 0] = float(0.0)

for i in xrange(Lr + 1, Lr + Lm, 1):  # first magma injection temperature.
    b[i, 0] = 1302.0

for i in xrange(Lr + Lm + 1, n, 1):
...
# Finite differences equations constants for upper crust
...
# Rock eq above TL below Ts

S = float((kr * dt) / (dc * Cc * dx * dx))  # S de la ecuacion de la roca
...
for j in xrange(1, Nt, 1):  # loop for solving system
    ...
    #  Heat transfer equations for Mantle with phase change possibility.

    for c in xrange(Lr + Lm + 1, n - 1, 1):
        if b[c, 0] > TLc:
            a[c, c] = float(1 + (2 * Sl))
            a[c, c + 1] = -Sl
            if c > 0:
    ...
    if j == 1601:
        ...
        for ip in xrange(Lr + 1, Lr + Lm, 1):  # first magma injection temperature.
            if b[ip, 0] > TL:
                fa[ip, 0] = 1
            elif b[ip, 0] < Ts:
                fa[ip, 0] = 0
    ...
    if j % DT == 0 and u < 320:
        ...
        # print n
        n = n + LI
        #print b[Lr + Lm - 1], b[Lr + Lm], b[Lr + Lm + 1], "este es b"


        a = np.zeros((n, n))
        o = np.zeros((n, 1))

        for y in xrange(0, Lr + Lm +1, 1):
            o[y] = b[y]
        for m in xrange(1, n, 1):
            o[Lr + Lm + m + 2] = b[Lr + Lm  + m]
            if Lr + Lm + m +2 == n - 1:
                break
        for z in xrange(Lr + Lm +1, Lr + Lm+ 3, 1):  # Temperature of new injection
            o[z] = float(1302.0)

       # print o[Lr + Lm - 1], o[Lr + Lm], o[Lr + Lm + 1],o[Lr+Lm+2],o[Lr+Lm+3], "este es o"
        Lm = Lm + LI
        ...
        # if u == 3:
        #  for di in xrange(Lr + Lm - LI - 30, Lr + Lm + 30, 1):
        #     print o[di]

        r += 50.0 # aumento en el grosor de la corteza

        u += 1  # Magma injection counting
#  Lo que pasa aqui es que cojo un punto y veo como evoluciona su fraccion de fusion y por lo tanto su tem.
    ...
    if u >= 50:
        if u==50:
            dT = Lr+Lm-LI+1
            p3 = b[dT]  # if there's a magma injection,
        else:
            p3 = b[dT]
        ...
        if p3 > TL:
            f2 = 1
            #print "este es lo qye necesita 1",u,f2,p3
        elif p3 < Ts:
            f2 = 0
            #print "este es lo qye necesita 2", u, f2, p3
        elif TL >= p3 >= Ta:
            f2 = ((3.25 * 10 ** (-3)) * (p3 - TL)) + 1
            #print "este es lo qye necesita 3", u, f2, p3
        elif Ta >= p3 >= Ts:
            f2 = (Xa / (Ta - Ts)) * (p3 - Ts)
            #print "este es lo qye necesita 4", u,  f2, p3
        if u >= 51:
            data2.append(p3)
